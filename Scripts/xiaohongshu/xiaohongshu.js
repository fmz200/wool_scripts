...
// 小红书画质增强：加载2K分辨率的图片
function imageEnhance(path) {
  console.log('图片画质增强开始♻');
  const regex1 = /imageView2\/2\/w\/\d+\/format/g;
  path = path.replace(regex1, `imageView2/2/w/2160/format`);

  const regex2 = /imageView2\/2\/h\/\d+\/format/g;
  path = path.replace(regex2, `imageView2/2/h/2160/format`);
