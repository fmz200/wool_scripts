...
if (body) {
  switch (!0) {
    ...
    case/api\/sns\/v\d\/system_service\/config\?/.test($request.url):
      try {
        let x = JSON.parse(body), C = ["store", "splash", "loading_img", "app_theme", "cmt_words", "highlight_tab"];
  ...
  body = body.replace(regex1, `imageView2/2/w/2160/format`);

  const regex2 = /imageView2\/2\/h\/\d+\/format/g;
  body = body.replace(regex2, `imageView2/2/h/2160/format`);
